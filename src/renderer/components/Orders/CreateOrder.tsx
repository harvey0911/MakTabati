...
const CreateOrder = ({ onBack, onOrderCreated }: { onBack: () => void, onOrderCreated: () => void }) => {
    ...
    const totalAmount = cart.reduce((sum, item) => sum + (item.quantity * item.product.cost_price), 0);
    ...
    const generatePdf = (orderId: number) => {
        ...
        doc.text(`Supplier / Notes: ${customerName || 'N/A'}`, 14, 59);

        const tableData = cart.map(item => [
            item.product.name,
            item.quantity.toString(),
            `$${item.product.cost_price.toFixed(2)}`,
            `$${(item.quantity * item.product.cost_price).toFixed(2)}`
        ]);
        ...
        <div style={{ display: 'flex', gap: '1.5rem', marginTop: '1rem', height: 'calc(100vh - 150px)' }}>
            ...
            <div className="card" style={{ flex: 6, padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
                    ...
                    <h3 style={{ fontSize: '1.125rem', fontWeight: 600, margin: 0 }}>Select Products</h3>
                </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
                        {filteredProducts.map(p => (
                            ...
                            >
                                ...
                                <div style={{ fontWeight: 600, color: '#10b981', marginTop: 'auto' }}>Cost: ${p.cost_price.toFixed(2)}</div>
                            </div>
                        ))}
            ...
            <div className="card" style={{ flex: 4, padding: '1.5rem', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--background)' }}>
                ...
                <div style={{ marginBottom: '1rem' }}>
                    <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 500, color: '#475569', marginBottom: '4px' }}>Supplier / Notes (Optional)</label>
                    <input
                        type="text"
                            ...
                            {cart.map(item => (
                                <div key={item.product.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>${item.product.cost_price.toFixed(2)} / each (Cost)</div>
                                    </div>
                ...
                <div style={{ marginTop: 'auto' }}>
                    ...
                    <button
                        ...
                        style={{
                            ...
                            backgroundColor: cart.length === 0 ? '#cbd5e1' : '#4f46e5',
