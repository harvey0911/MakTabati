...
import { Plus, Minus, Trash2, ArrowLeft, ShoppingCart, Search, Package } from 'lucide-react';
...
const SellView = ({ onComplete }: { onComplete: () => void }) => {
    ...
    const addToCart = (product: Product) => {
        if (product.total_stock <= 0) {
            return showAlert('Out of Stock', `${product.name} has no available stock.`, 'error');
        }

        const existing = cart.find(item => item.product.id === product.id);
        if (existing) {
            if (existing.quantity >= product.total_stock) {
                return showAlert('Stock Limit', `Cannot add more than ${product.total_stock} of ${product.name}.`, 'error');
            }
    ...
    const updateQuantity = (productId: number, newQty: number) => {
        ...
        if (newQty > item.product.total_stock) {
            return showAlert('Stock Limit', `Cannot add more than ${item.product.total_stock} of ${item.product.name}.`, 'error');
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.error(error);
            showAlert('Error', 'Failed to process sale.', 'error');
        } finally {
        ...
        <div style={{ display: 'flex', gap: '1.5rem', height: 'calc(100vh - 120px)', padding: '0.5rem' }}>
            ...
            <div className="card" style={{ flex: 7, padding: '1.5rem', display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
                    ...
                    <h3 style={{ fontSize: '1.125rem', fontWeight: 600, margin: 0 }}>Product Selection</h3>
                </div>
                        ...
                        {filteredProducts.map(p => (
                            <div
                                ...
                                style={{
                                    ...
                                    opacity: p.total_stock <= 0 ? 0.6 : 1,
                                    borderWidth: '2px'
                                }}
                                onMouseOver={(e) => { if (p.total_stock > 0) e.currentTarget.style.borderColor = '#4f46e5'; }}
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <span style={{ fontWeight: 700, color: '#10b981' }}>${p.sell_price.toFixed(2)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="card" style={{ flex: 5, padding: '1.5rem', display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1.5rem' }}>
                    ...
                    <h3 style={{ fontSize: '1.125rem', fontWeight: 600, margin: 0 }}>Current Sale</h3>
                </div>
                ...
                <div style={{ marginTop: 'auto', backgroundColor: 'white', padding: '1.25rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem', alignItems: 'baseline' }}>
                        ...
                        <span style={{ fontSize: '1.75rem', fontWeight: 700, color: '#10b981' }}>${totalAmount.toFixed(2)}</span>
                    </div>
                    ...
                    <button
                        ...
                        style={{
                            ...
                            boxShadow: cart.length === 0 ? 'none' : '0 4px 6px -1px rgba(16, 185, 129, 0.2)'
                        }}>
